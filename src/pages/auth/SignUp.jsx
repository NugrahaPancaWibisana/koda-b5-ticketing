...
import facebook from "../../assets/bx_bxl-facebook-circle.svg";
...
export default function SignUp() {
  ...
  const { email, password, terms } = {
    email: useInput("", { required: true }),
    password: useInput("", { required: true, minLength: 8 }),
    terms: useInput(false, { required: true }),
  };
  ...
  useEffect(() => {
    if (Object.keys(data).length !== 0) {
      dispatch(userActions.addUserThunk(data));
    }
  }, [data, dispatch]);
  ...
  return (
    <section className="flex min-h-dvh flex-col items-center justify-center py-10">
      ...
      <form
        className="flex w-[90%] flex-col items-center justify-center gap-6 rounded-lg bg-white p-10 sm:max-w-120"
        onSubmit={onSubmitHandler}
      >
        <div className="flex w-full items-center justify-between gap-5">
          <div className="flex flex-col items-center justify-center gap-2">
            <p className="bg-primary flex h-10 w-10 items-center justify-center rounded-full leading-0 text-white">
              1
            </p>
            <p className="w-full">Fill form</p>
          </div>
          <hr className="border-tertiary flex-1 border-t border-dashed" />
          <div className="flex flex-col items-center justify-center gap-2">
            <p className="bg-tertiary flex h-10 w-10 items-center justify-center rounded-full leading-0 text-white">
              2
            </p>
            <p>Activate</p>
          </div>
        ...
        <div className="w-full">
          ...
          <input
            className="border-tertiary h-14 w-full rounded-sm border pl-3 outline-0"
            type="email"
            name="email"
            id="email"
            autoComplete="email"
        ...
        <div className="w-full">
          ...
          <div className="border-tertiary flex rounded-sm border px-3">
            <input
              className="h-14 w-full outline-0"
            ...
            >
              {isShow ? (
                <img className="h-5 w-5" src={eyeOpen} alt="eye closed" />
              ) : (
                <img className="h-5 w-5" src={eyeClosed} alt="eye closed" />
              )}
            </button>
          </div>
          <div className="h-2">
            {!password.isValid && (
              <p className="text-sm text-red-600">{password.error}</p>
            )}
          </div>
        </div>
        <div className="w-full">
          <input
            ...
            onChange={terms.onChangeHandler}
          />
          <label className="text-tertiary" htmlFor="terms">
        ...
        <button
          ...
          className={`${
            isFormValid ? "bg-primary" : "bg-primary/30 cursor-not-allowed"
          } h-14 w-full rounded-sm text-white`}
        >
          Join For Free Now
        </button>
        <p>
          Already have an account?{" "}
          <Link to={"/signin"} className="text-blue-700" href="#">
            Log in
          </Link>
        </p>
        <div className="text-tertiary flex w-full items-center justify-center gap-5">
          <hr className="border-tertiary w-full border-t" />
          <p>or</p>
          <hr className="border-tertiary w-full border-t" />
        </div>
        <div className="flex w-full gap-5">
          <button
            className="flex h-14 w-full items-center justify-center gap-2 shadow"
            type="button"
          >
