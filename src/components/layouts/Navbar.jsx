...
export default function Navbar() {
  ...
  useEffect(() => {
    if (logout) {
      dispatch(userActions.userLogoutThunk());
    }
  }, [dispatch, logout]);
        ...
        <nav className="flex w-full items-center justify-between md:px-[30px] lg:px-[130px]">
          <div className="flex w-full items-center justify-between px-6 md:w-fit md:px-0">
            ...
            <button
              ...
              className="flex h-6 w-6 flex-col items-end justify-between p-1 md:hidden"
            >
              <span
                className={`${
                  isOpen ? "w-[21px] transform-[rotate(-45deg)]" : ""
                } block h-0.5 w-4 origin-top-right bg-black transition-all`}
              />
              <span
                className={`${
                  isOpen ? "opacity-0" : ""
                } block h-0.5 w-2.5 bg-black transition-all`}
              />
              <span
                className={`${
                  isOpen ? "w-[21px] transform-[rotate(45deg)]" : ""
                } block h-0.5 w-4 origin-bottom-right bg-black transition-all`}
              />
            </button>
          </div>
          <div
          ...
          >
            <ul className="flex flex-col gap-2">
              <li>
                <NavLink className="text-xl font-normal" to="/">
                  Home
                </NavLink>
              </li>
              <li>
                <NavLink className="text-xl font-normal" to="/movies">
                  Movie
                </NavLink>
              </li>
              <li>
            ...
            <ul className="flex w-full gap-3">
              {Object.keys(user.user_active).length === 0 ? (
                <>
                  <li className="flex-1">
                    <NavLink
                      className="border-primary text-primary block rounded-lg border py-3 text-center"
                      to="/signin"
                    >
                      Sign In
                    </NavLink>
                  ...
                  <li className="flex-1">
                    <NavLink
                      className="bg-primary block rounded-lg py-3 text-center text-white"
                      to="/signup"
                    >
                      Sign Up
                    </NavLink>
                ...
                <li className="relative flex-1 rounded-lg border">
                  <div
                    className="flex items-center justify-evenly py-1"
                    onClick={() => setDropDown(!dropdown)}
                  >
                    <img
                      className="h-10 w-10 rounded-full bg-cover bg-center"
                      src={user.user_active.image}
                      alt={user.user_active.email}
                    />
                    <p>
                      {/* {user.user_active.first_name} {user.user_active.last_name} */}
                      {user.user_active.email}
                    </p>
                  </div>
                  {dropdown && (
                    <div className="absolute top-0 left-1/2 flex w-full -translate-x-1/2 -translate-y-22 flex-col items-start gap-1 rounded-lg border px-5 py-2">
          ...
          <ul className="hidden gap-2 md:flex md:flex-row md:gap-[60px]">
            <li>
              <NavLink className="text-xl font-normal" to="/">
                Home
              </NavLink>
          ...
          <ul className="hidden gap-3 md:flex md:gap-3">
            {Object.keys(user.user_active).length === 0 ? (
              <>
                <li>
                  <NavLink
                    className="border-primary text-primary block rounded-lg border py-3 text-center md:px-5"
                    to="/signin"
                  >
                    Sign In
                  </NavLink>
                ...
                <li>
                  <NavLink
                    className="bg-primary block rounded-lg py-3 text-center text-white md:px-5"
                    to="/signup"
                  >
                    Sign Up
                  </NavLink>
              ...
              <li className="relative">
                ...
                />
                {dropdown && (
                  <div className="absolute left-1/2 flex -translate-x-1/2 translate-y-3 flex-col gap-1 rounded-lg border bg-white px-5 py-2">
                    <NavLink to="#" className="text-lg">
                      Profile
                    </NavLink>
                    <button
